# GO2-W Nav2 可选影子实验（默认停用）

本工作空间在不更换宇树底层系统、LIO-SAM、SCAN-Planner 或控制协议的前提下，
保留一套 ROS 2 Humble Nav2 实验链供离线对比。默认整机启动不会运行 Nav2；
实际导航使用 Web A*、SCAN 局部规划、自研闭环控制器和实时脱困节点。若手工启动
Nav2 影子服务，底盘安全门仍只转发 SCAN。Nav2 不使用 TensorRT，也不修改
GO2-W 固件。
...
`start_navigation.sh` 会主动停止 Nav2 影子服务并选择 `scan` 控制后端。只有手工
执行 `start_nav2_shadow.sh` 时才会启动这套实验链。
