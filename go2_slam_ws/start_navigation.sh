...
# GO2-W 重启后一键拉起完整导航链路。
#
# 顺序：XT16 -> 内置 IMU/Web -> LIO-SAM -> SCAN-Planner。
...
echo "==> [1/6] 清理未执行任务的旧 SCAN 实例"
"$SCAN_WS/stop_go2w.sh" >/dev/null 2>&1 || true

echo "==> [2/6] 拉起 XT16、内置 IMU/Web 和 LIO-SAM"
if $BUILD; then
