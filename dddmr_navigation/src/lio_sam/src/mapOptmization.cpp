...
public:
    ...
    {
        // Web 直接消费回环修正后的全局关键帧地图；1Hz 足够实时且避免 Python
        // 逐帧累积器无法应用回环修正的问题。
    ...
    void updateInitialGuess()
    {
        ...
        // use imu pre-integration estimation for pose guess. On the GO2-W the
        // lidar is slow (10 Hz) and no wheel odometry is fused here; an IMU
        // acceleration residual can otherwise inject metres of false translation
        // while the robot is stationary. The platform config therefore uses the
        // raw IMU rotation-only branch below and lets scan-to-map solve translation.
        static bool lastImuPreTransAvailable = false;
        static Eigen::Affine3f lastImuPreTransformation;
        if (useImuPreintegrationInitialGuess && cloudInfo.odom_available == true)
        {
