...
namespace scan_planner
{
  ...
  private:
    ...
    ControlPoints cps_;
    // initControlPoints used to return an empty A* path both when there was no
    // collision and when collision recovery failed.  Keep an explicit status
    // so a failed obstacle search can never fall through to trajectory publish.
    bool control_points_collision_valid_{true};
    // Keep the pre-A-star reference for collision-checked detour expansion.
    // A cubic B-spline does not interpolate its control polygon: around a
    // tight corner it can cut back inside an obstacle even when every A-star
    // control point is free.  rebound_optimize() uses this reference to move
    // the detour farther from the obstructed reference, accepting a candidate
    // only after a dense full-curve collision check.
    Eigen::MatrixXd astar_seed_reference_points_;
    bool astar_seed_generated_{false};
