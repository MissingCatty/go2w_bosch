...
struct MappingParameters {
  ...
  double occupancy_decay_time_, occupancy_decay_interval_,
      occupancy_decay_sensor_timeout_, occupancy_decay_keep_radius_,
      occupancy_decay_near_max_time_;
  ...
  bool use_static_navigation_grid_;
...
struct MappingData {
  ...
  std::vector<double> occupancy_last_hit_time_;
  std::vector<char> occupancy_decay_active_;
  std::vector<uint32_t> occupancy_decay_generation_;
  std::vector<std::pair<int, uint32_t>> occupancy_decay_active_entries_;
  ...
  double last_observation_time_, last_fusion_time_, last_decay_time_;
  uint64_t fusion_sequence_;
...
public:
  ...
  bool odomValid();
  bool hasFreshObservation(double max_age) const;
  uint64_t observationSequence() const;
...
private:
  ...
  void applyStaticOccupancy(const std::vector<char>* address_mask = nullptr);
  int getStaticNavigationOccupancy(const Eigen::Vector3d& pos) const;
  ...
  void raycastProcess();
  void decayStaleOccupancy(double now);
  ...
  bool static_navigation_grid_ready_{false};
  double static_navigation_grid_resolution_{0.0};
  double static_navigation_grid_origin_x_{0.0};
  double static_navigation_grid_origin_y_{0.0};
  double static_navigation_grid_origin_cos_{1.0};
  double static_navigation_grid_origin_sin_{0.0};
  uint32_t static_navigation_grid_width_{0};
  uint32_t static_navigation_grid_height_{0};
  std::vector<int8_t> static_navigation_grid_data_;
  vector<Eigen::Vector3d> static_points_;
  bool static_cloud_initialized_{false};
...
inline int GridMap::getInflateOccupancy(Eigen::Vector3d pos, double yaw) {
  ...
  int rear_occ = getInflateOccupancyFromBuffer(rear, md_.occupancy_buffer_inflate_);
  if (rear_occ != 0) return rear_occ;

  // The saved grid is a global wall/topology prior and is already inflated by
  // the 0.23 m cylinder radius.  Query only the body centre in that prior.
  // Whether the complete long body can rotate/pass *now* is decided above by
  // the live lidar layer using both cylinder centres.  This avoids rejecting
  // a real, currently clear turn because a saved wall edge or registration is
  // off by a few centimetres, while a missing lidar return still cannot make
  // the route centre cross a saved wall.
  return getStaticNavigationOccupancy(pos);
}
