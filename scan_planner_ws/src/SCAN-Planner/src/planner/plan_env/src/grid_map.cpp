...
void GridMap::initMap(rclcpp::Node *node)
{
  ...
  load_parameter(node_, "grid_map.occupancy_decay_time", mp_.occupancy_decay_time_, 0.0);
  load_parameter(node_, "grid_map.occupancy_decay_interval", mp_.occupancy_decay_interval_, 0.5);
  load_parameter(node_, "grid_map.occupancy_decay_sensor_timeout",
                 mp_.occupancy_decay_sensor_timeout_, 0.6);
  load_parameter(node_, "grid_map.occupancy_decay_keep_radius",
                 mp_.occupancy_decay_keep_radius_, 0.8);
  load_parameter(node_, "grid_map.occupancy_decay_near_max_time",
                 mp_.occupancy_decay_near_max_time_, 6.0);
  ...
  mp_.occupancy_decay_time_ = std::max(0.0, mp_.occupancy_decay_time_);
  mp_.occupancy_decay_interval_ = std::max(0.05, mp_.occupancy_decay_interval_);
  mp_.occupancy_decay_sensor_timeout_ =
      std::max(0.1, mp_.occupancy_decay_sensor_timeout_);
  mp_.occupancy_decay_keep_radius_ =
      std::max(0.0, mp_.occupancy_decay_keep_radius_);
  mp_.occupancy_decay_near_max_time_ = std::max(
      mp_.occupancy_decay_time_, mp_.occupancy_decay_near_max_time_);
  ...
  if (mp_.occupancy_decay_time_ > 0.0)
    RCLCPP_INFO(node_->get_logger(),
                "[GridMap] live obstacle expiry enabled: stale=%.1fs, "
                "protected radius=%.2fm for at most %.1fs, sensor timeout=%.1fs",
                mp_.occupancy_decay_time_,
                mp_.occupancy_decay_keep_radius_,
                mp_.occupancy_decay_near_max_time_,
                mp_.occupancy_decay_sensor_timeout_);
  ...
  md_.occupancy_last_hit_time_ = vector<double>(buffer_size, 0.0);
  md_.occupancy_decay_active_ = vector<char>(buffer_size, 0);
  md_.occupancy_decay_generation_ = vector<uint32_t>(buffer_size, 0);
  md_.occupancy_decay_active_entries_.reserve(buffer_size / 16);
  ...
  else if (mp_.sensor_type_ == "lidar")
  {
    ...
    {
      auto static_qos = rclcpp::QoS(1).reliable().transient_local();
      static_cloud_sub_ = node_->create_subscription<sensor_msgs::msg::PointCloud2>(
          "static_cloud", static_qos,
  ...
  if (mp_.use_static_navigation_grid_)
  {
    auto static_grid_qos = rclcpp::QoS(1).reliable().transient_local();
    static_navigation_grid_sub_ =
        node_->create_subscription<nav_msgs::msg::OccupancyGrid>(
            "static_navigation_grid", static_grid_qos,
            std::bind(&GridMap::staticNavigationGridCallback, this,
                      std::placeholders::_1));
  }
  ...
  md_.last_observation_time_ = 0.0;
  md_.last_fusion_time_ = 0.0;
  md_.last_decay_time_ = 0.0;
  md_.fusion_sequence_ = 0;
  md_.image_cnt_ = 0;
...
void GridMap::resetAllMapData()
{
  ...
  std::fill(md_.occupancy_last_hit_time_.begin(), md_.occupancy_last_hit_time_.end(), 0.0);
  std::fill(md_.occupancy_decay_active_.begin(), md_.occupancy_decay_active_.end(), 0);
  md_.occupancy_decay_active_entries_.clear();
...
void GridMap::resetCellByAddress(int addr)
{
  ...
  md_.occupancy_last_hit_time_[addr] = 0.0;
  md_.occupancy_decay_active_[addr] = 0;
  ++md_.occupancy_decay_generation_[addr];
...
void GridMap::updateSlidingMap(const Eigen::Vector3d& center)
{
  ...
  if ((shift_num.cwiseAbs().array() >= mp_.map_voxel_num_.array()).any())
  {
    ...
    md_.local_bound_min_ = mp_.map_bound_min_idx_;
    md_.local_bound_max_ = mp_.map_bound_max_idx_;
    // The static map is a prior, not an immutable obstacle layer.  Seed it
    // after a full sliding-map reset, then let subsequent live lidar misses
    // clear stale objects such as people or moved furniture.
    applyStaticOccupancy();
    return;
  }
  ...
  // Only seed cells newly exposed by the circular-buffer shift.  Reapplying
  // every static point here (or on every sensor pose) prevents ray casting
  // from ever clearing an obsolete static obstacle.
  applyStaticOccupancy(&clear_mask);
}
...
int GridMap::setCacheOccupancy(Eigen::Vector3d pos, int occ)
{
  ...
  // Preserve every live lidar hit, including hits that overlap a saved wall.
  // The static grid now protects only the route centre; the current full-body
  // collision shape is intentionally supplied by these clearable live hits.
  // Suppressing wall-overlap hits would leave the front/rear cylinders with
  // no real-time evidence exactly where it matters most.
  ...
  if (occ == 1)
  {
    ...
    // Only a real sensor endpoint extends a live obstacle's lifetime. Ray
    // misses, footprint inflation and saved-map priors never refresh it.
    md_.occupancy_last_hit_time_[idx_ctns] = md_.last_observation_time_;
    if (mp_.occupancy_decay_time_ > 0.0 && !mp_.use_static_map_ &&
        !md_.occupancy_decay_active_[idx_ctns])
    {
      md_.occupancy_decay_active_[idx_ctns] = 1;
      const uint32_t generation = ++md_.occupancy_decay_generation_[idx_ctns];
      md_.occupancy_decay_active_entries_.emplace_back(idx_ctns, generation);
    }
  }
...
void GridMap::updateOccupancyCallback()
{
  ...
  // t2 = ros::Time::now();
  raycastProcess();
  // Static walls are authoritative and must not be erased by a sparse live
  // ray that happens to continue through a saved-map obstacle.
  applyStaticOccupancy();
  md_.last_fusion_time_ = now;
  ++md_.fusion_sequence_;
  // t3 = ros::Time::now();
...
void GridMap::decayStaleOccupancy(double now)
{
  if (mp_.occupancy_decay_time_ <= 0.0 || mp_.use_static_map_ ||
      md_.last_observation_time_ <= 0.0 ||
      now - md_.last_observation_time_ > mp_.occupancy_decay_sensor_timeout_ ||
      now - md_.last_decay_time_ < mp_.occupancy_decay_interval_)
    return;

  md_.last_decay_time_ = now;
  size_t expired = 0;
  std::vector<std::pair<int, uint32_t>> retained;
  retained.reserve(md_.occupancy_decay_active_entries_.size());
  for (const auto &[addr, generation] : md_.occupancy_decay_active_entries_)
  {
    if (!md_.occupancy_decay_active_[addr] ||
        md_.occupancy_decay_generation_[addr] != generation)
      continue;

    const double last_hit = md_.occupancy_last_hit_time_[addr];
    if (last_hit > 0.0 && now - last_hit < mp_.occupancy_decay_time_)
    {
      retained.emplace_back(addr, generation);
      continue;
    }

    // Never time-expire a close obstacle that may have entered the lidar's
    // body/blind zone.  It must be explicitly cleared by free-space rays or
    // leave the rolling map.  Farther points are visible to the live sensor;
    // if they have not been hit again for the configured short memory, they
    // are stale scan artifacts or a dynamic object that has moved away.
    Eigen::Vector3i id;
    hashIdToGlobalIndex(addr, id);
    Eigen::Vector3d position;
    indexToPos(id, position);
    if ((position.head<2>() - md_.ray_pos_.head<2>()).norm() <
            mp_.occupancy_decay_keep_radius_ &&
        last_hit > 0.0 &&
        now - last_hit < mp_.occupancy_decay_near_max_time_)
    {
      retained.emplace_back(addr, generation);
      continue;
    }

    if (last_hit > 0.0 && md_.occupancy_buffer_[addr] > mp_.min_occupancy_log_)
    {
  ...
  md_.occupancy_decay_active_entries_.swap(retained);

  if (expired > 0)
    RCLCPP_INFO_THROTTLE(
        node_->get_logger(), *node_->get_clock(), 10000,
        "[GridMap] expired %zu live obstacle voxels not re-observed for %.1fs",
        expired, mp_.occupancy_decay_time_);
}
                                ...
                                const nav_msgs::msg::Odometry::ConstSharedPtr &pose)
{
  ...
  if (isInMap(md_.ray_pos_))
  {
    ...
    md_.update_num_ += 1;
    md_.last_observation_time_ = node_->now().seconds();
...
void GridMap::staticCloudCallback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &cloud_msg)
{
  ...
  const bool first_static_cloud = !static_cloud_initialized_;
  pcl::PointCloud<pcl::PointXYZ> cloud;
  pcl::fromROSMsg(*cloud_msg, cloud);
  static_points_.clear();
  static_points_.reserve(cloud.points.size());
  for (const auto &point : cloud.points)
  {
    if (std::isfinite(point.x) && std::isfinite(point.y) && std::isfinite(point.z))
      static_points_.emplace_back(point.x, point.y, point.z);
  }
  // The bridge republishes its transient-local cloud periodically.  Re-seeding
  // the same prior on every message would resurrect obstacles that live lidar
  // has correctly cleared.  A planner restart or a sliding-window refill is
  // the explicit boundary for applying the prior again.
  if (first_static_cloud)
  {
    applyStaticOccupancy();
    static_cloud_initialized_ = true;
    RCLCPP_INFO(node_->get_logger(), "[GridMap] loaded %zu static obstacle points",
                static_points_.size());
  }
}

void GridMap::staticNavigationGridCallback(
    const nav_msgs::msg::OccupancyGrid::ConstSharedPtr &grid)
{
  if (!mp_.use_static_navigation_grid_)
    return;

  const bool first_valid_grid = !static_navigation_grid_ready_;
      ...
      grid->data.size() != static_cast<size_t>(width) * height)
  {
    RCLCPP_ERROR(node_->get_logger(),
                 "[GridMap] rejected malformed static navigation grid");
    static_navigation_grid_ready_ = false;
    return;
  }

  if (!grid->header.frame_id.empty() && grid->header.frame_id != mp_.frame_id_)
  {
    RCLCPP_ERROR(node_->get_logger(),
                 "[GridMap] static navigation grid frame '%s' does not match '%s'",
                 grid->header.frame_id.c_str(), mp_.frame_id_.c_str());
    static_navigation_grid_ready_ = false;
    return;
  }
  ...
  static_navigation_grid_resolution_ = resolution;
  static_navigation_grid_origin_x_ = grid->info.origin.position.x;
  static_navigation_grid_origin_y_ = grid->info.origin.position.y;
  static_navigation_grid_origin_cos_ = std::cos(yaw);
  static_navigation_grid_origin_sin_ = std::sin(yaw);
  static_navigation_grid_width_ = width;
  static_navigation_grid_height_ = height;
  static_navigation_grid_data_ = grid->data;
  static_navigation_grid_ready_ = true;

  if (first_valid_grid)
    RCLCPP_INFO(node_->get_logger(),
                "[GridMap] loaded immutable inflated navigation grid: %ux%u at %.3fm",
                width, height, resolution);
}

int GridMap::getStaticNavigationOccupancy(const Eigen::Vector3d &pos) const
{
  if (!mp_.use_static_navigation_grid_)
    return 0;
  // Fail closed until the transient-local map arrives. This prevents the
  // widened local corridor from becoming an unbounded wall-crossing window
  // during startup or after a map publisher failure.
  if (!static_navigation_grid_ready_)
    return -1;

  const double dx = pos(0) - static_navigation_grid_origin_x_;
  const double dy = pos(1) - static_navigation_grid_origin_y_;
  const double local_x = static_navigation_grid_origin_cos_ * dx +
                         static_navigation_grid_origin_sin_ * dy;
  const double local_y = -static_navigation_grid_origin_sin_ * dx +
                          static_navigation_grid_origin_cos_ * dy;
  const int x = static_cast<int>(std::floor(
      local_x / static_navigation_grid_resolution_));
  const int y = static_cast<int>(std::floor(
      local_y / static_navigation_grid_resolution_));
  if (x < 0 || y < 0 ||
      x >= static_cast<int>(static_navigation_grid_width_) ||
      y >= static_cast<int>(static_navigation_grid_height_))
    return -1;

  const int8_t value = static_navigation_grid_data_[
      static_cast<size_t>(y) * static_navigation_grid_width_ + x];
  // Unknown cells use -1 and are deliberately not a detour area. This is the
  // same free-space contract used by the Web global planner.
  return value == 0 ? 0 : 1;
}

void GridMap::applyStaticOccupancy(const std::vector<char>* address_mask)
{
  if (!mp_.use_static_map_ || static_points_.empty())
    return;
...
bool GridMap::hasFreshObservation(double max_age) const
{
  if (!node_ || md_.last_fusion_time_ <= 0.0 || max_age <= 0.0)
    return false;
  const double age = node_->now().seconds() - md_.last_fusion_time_;
  return age >= 0.0 && age <= max_age;
}

uint64_t GridMap::observationSequence() const { return md_.fusion_sequence_; }
