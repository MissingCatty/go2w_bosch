...
bool AStar::ConvertToIndexAndAdjustStartEndPoints(Vector3d start_pt, Vector3d end_pt, Vector3i &start_idx, Vector3i &end_idx)
{
    ...
    if ((start_idx - end_idx).squaredNorm() == 0)
        return false;

    // Do not classify the endpoints using the straight start-to-end yaw.  A
    // collision segment may wrap a corner, so that chord can rotate the
    // capsule into a wall even though both endpoints are valid with their
    // actual local tangents.  Every neighbor, including the step that reaches
    // the end node, is already footprint-checked with its movement yaw in the
    // search loop.  Leaving the start node unchecked also lets a robot escape
    // a conservative/temporarily overlapping live cell instead of pushing
    // the endpoint blindly away from the route.
    return true;
}
